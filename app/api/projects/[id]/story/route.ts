...
export async function POST(request: Request, context: Context) {
  try {
    ...
    else if (action === "revise") next = reviseStory(project, String(body.notes || ""));
    else if (action === "approve") next = approveStory(project, String(body.text || ""));
    else if (action === "reopen") next = reopenStory(project);
    else if (action === "rebrief") {
      assertBriefAllowed(String(body.brief || ""));
      next = updateBrief(project, String(body.brief || ""));
